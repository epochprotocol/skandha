...
import { Log, Logger } from "../interfaces";
import { MempoolService, UserOpValidationService } from "../services";
...
export class Eth {
	...
	async sendUserOperation(args: SendUserOperationGasArgs): Promise<string> {
		...
		console.log(
			"userOp.advancedUserOperation: ",
			userOp.advancedUserOperation
		);
		if (!userOp.advancedUserOperation?.readyForExecution) {
	...
	async getUserOperations(
		...
		const advancedMempoolEntry: Array<AdvancedOpMempoolEntry> =
			await this.advancedMempoolService.fetchAllConditional(
				timebasedContions
			);
