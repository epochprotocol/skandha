...
export class AdvancedOperationMempoolService {
  ...
  constructor(private db: IDbController, private chainId: number) {
  ...
  async addAdvancedUserOp(
    ...
    await this.db.put(
      this.ADVANCED_USEROP_COLLECTION_KEY,
      newAdvancedUserOpKeys
    );
    await this.db.put(key, { ...entry, lastUpdatedTime: now() });
    // }
  }
  ...
  private async find(
    entry: AdvancedOpMempoolEntry
  ): Promise<AdvancedOpMempoolEntry | null> {
  ...
  public async fetchAllConditional(
    conditions: Array<Conditions>
  ): Promise<AdvancedOpMempoolEntry[]> {
    console.log("conditions2: ", conditions);
    const keys = await this.fetchKeys();
    console.log("keys2: ", keys);
    const rawEntries = await this.db
      .findConditional(conditions, keys)
      .catch(() => []);
    return rawEntries.map(this.rawEntryToMempoolEntry);
  }

  private rawEntryToMempoolEntry(
    raw: AdvancedOpMempoolEntry
  ): AdvancedOpMempoolEntry {
  ...
  public async fetchAllEventConditionals(
