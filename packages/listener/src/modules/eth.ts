...
export class Eth {
    ...
    public onBlockCallback = async (
        ...
        const newAdvancedMempoolEntry: Array<AdvancedOpMempoolEntry> = advancedMempoolEntry.filter((val) => !val.userOp.advancedUserOperation?.readyForExecution)
        this.logger.info("advancedMempoolEntry", newAdvancedMempoolEntry)
        ...
        await Promise.all(newAdvancedMempoolEntry.map(async (element) => {
            // await this.advancedMempoolService.remove(element);
            await this.advancedTransactionToMempool(element)
    ...
    private advancedTransactionToMempool = async (advancedMempoolEntry: AdvancedOpMempoolEntry) => {
